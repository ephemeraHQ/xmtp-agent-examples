...
export class WalletService {
  ...
  async createWallet(key: string): Promise<AgentWalletData> {
    try {
      ...
      if (!apiKeyName || !apiKeyPrivate) {
        throw new Error(
          "Coinbase SDK credentials not properly configured. Check your environment variables.",
        );
      }

      // Log the network we're using
      console.log(
        `Creating wallet on network: ${Coinbase.networks.BaseSepolia}`,
      );
      ...
      const wallet = await Wallet.create({
        ...
        console.error(
          "Detailed wallet creation error:",
      ...
      await this.walletStorage.set(`wallet:${key}`, JSON.stringify(walletInfo));
      ...
      if (error instanceof Error) {
        if (error.message && error.message.includes("API")) {
          console.error(
            "API Error detected, check credentials and network configuration",
          );
          throw new Error(
            `Coinbase API error: Check that your API keys are correctly formatted and have the necessary permissions. Original error: ${error.message}`,
          );
        } else if (error.message && error.message.includes("network")) {
          console.error("Network-related error detected");
          throw new Error(
            `Network error creating wallet: ${error.message}. Check your internet connection and Coinbase API status.`,
          );
        } else if (error.message && error.message.includes("permission")) {
          console.error("Permission-related error detected");
          throw new Error(
            `Permission error: Your API key may not have the necessary permissions. Original error: ${error.message}`,
          );
        } else {
  ...
  async getWallet(
    ...
    try {
      ...
      const decrypted = JSON.parse(walletData) as AgentWalletData;

      console.log("Importing wallet from stored data...");
      const importedWallet = await Wallet.import(decrypted.data).catch(
        (err: unknown) => {
          console.error("Error importing wallet:", err);
          throw err; // Rethrow to catch in outer catch block
        },
      );

      const walletId = importedWallet.getId() || decrypted.id;
      ...
      return {
        ...
        data: decrypted.data,
        human_address: decrypted.human_address,
  ...
  onRampURL(amount: number, address: string): string {
    if (!appId) return "";
  ...
  async transfer(
    ...
    // Check if amount is valid (not zero, not NaN)
