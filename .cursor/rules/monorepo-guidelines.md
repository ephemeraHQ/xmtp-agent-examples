...
cd examples/xmtp-gm
yarn test-manager send dev 0x4aDb8e00B5daC9EBda50c2b339290366DaE14EE0 'Hello XMTP world!'

# Expected terminal 1 output
Sending "gm" response to 0x94dafa657d01247ff6094567eb54bdd2baf16c10...
Waiting for messages...
```

For parallel testing with multiple instances:
...
### Test-manager command reference

| Command                                          | Description                                 |
| ------------------------------------------------ | ------------------------------------------- |
| `yarn test-manager start [count]`                | Start multiple agent instances (default: 2) |
| `yarn test-manager stop`                         | Stop all agent instances                    |
| `yarn test-manager status`                       | Show status of all running instances        |
| `yarn test-manager logs agent-1`                 | Show logs for a specific instance           |
| `yarn test-manager send dev <address> 'message'` | Send a test message                         |
